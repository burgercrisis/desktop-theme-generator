...
export const generateVariants = (
  ...
  prevHsl?: HSL,
  nextHsl?: HSL,
  contrastIntensity: number = 50 // New parameter for dynamic contrast control
): ColorStop[] => {
  ...
  for (let i = steps; i > 0; i--) {
    ...
    // Use dynamic contrast control instead of simple linear scaling
    const dynamicLightness = contrastControl.calculateVariantLightness(
      l, i - 1, steps, contrast, contrastIntensity, false // Light mode variants
    );
    stop.l = dynamicLightness;
  ...
  for (let i = 1; i <= steps; i++) {
    ...
    // Use dynamic contrast control instead of simple linear scaling
    const dynamicLightness = contrastControl.calculateVariantLightness(
      l, i - 1, steps, contrast, contrastIntensity, true // Dark mode variants
    );
    stop.l = dynamicLightness;
