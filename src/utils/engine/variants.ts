...
export const generateVariants = (
  ...
  for (let i = steps; i > 0; i--) {
    ...
    if (strategy === VariantStrategy.TINTS_SHADES) {
      ...
      const isDark = l < 50;
      stop.l = isDark ? Math.max(10, l - (10 * t)) : Math.min(90, l + (10 * t));
      stop.s = Math.max(10, s - (10 * t));
    } else if (strategy === VariantStrategy.NORDIC) {
      const targetH = 210;
      let dH = targetH - h;
      if (dH > 180) dH -= 360; else if (dH < -180) dH += 360;
      stop.h = normalizeHue(h + dH * (t * 0.2));
      stop.s = Math.max(5, s - (20 * t));
      stop.l = Math.max(20, l - (10 * t));
    } else if (strategy === VariantStrategy.DRACULA) {
      const targetH = 250;
      let dH = targetH - h;
      if (dH > 180) dH -= 360; else if (dH < -180) dH += 360;
      stop.h = normalizeHue(h + dH * (t * 0.2));
      stop.s = Math.max(20, s + (10 * t));
      stop.l = Math.max(10, l - (20 * t));
    } else if (strategy === VariantStrategy.MONOKAI) {
      const targetH = 330;
      let dH = targetH - h;
      if (dH > 180) dH -= 360; else if (dH < -180) dH += 360;
      stop.h = normalizeHue(h + dH * (t * 0.2));
      stop.s = Math.max(40, s + (15 * t));
      stop.l = Math.max(15, l - (15 * t));
    } else if (strategy === VariantStrategy.GRUVBOX) {
      const targetH = 35;
      let dH = targetH - h;
      if (dH > 180) dH -= 360; else if (dH < -180) dH += 360;
      stop.h = normalizeHue(h + dH * (t * 0.2));
      stop.s = Math.max(20, s - (5 * t));
      stop.l = Math.max(10, l - (10 * t));
    ...
    if (strategy === VariantStrategy.TINTS_SHADES) {
      ...
      stop.l = Math.min(100, l + ((100 - l) * t));
       ...
       stop.l = l > 50 ? Math.min(90, l + 50*t) : Math.max(10, l - 50*t);
        ...
        const isDark = l < 50;
        stop.l = isDark ? Math.min(90, l + (10 * t)) : Math.max(10, l - (10 * t));
        stop.s = Math.max(10, s - (10 * t));
      } else if (strategy === VariantStrategy.NORDIC) {
        const targetH = 210;
        let dH = targetH - h;
