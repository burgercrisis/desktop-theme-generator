...
const ThemePreview: React.FC<ThemePreviewProps> = ({ theme: rawTheme }) => {
  ...
  const theme = React.useMemo(() => {
    ...
    return {
      ...
      surfaceBrandBase: get('surface-brand-base'),
      surfaceBrandHover: get('surface-brand-hover'),
      surfaceInteractiveBase: get('surface-interactive-base'),
      surfaceInteractiveHover: get('surface-interactive-hover'),
      surfaceInteractiveWeak: get('surface-interactive-weak'),
      surfaceSuccessBase: get('surface-success-base'),
      surfaceSuccessWeak: get('surface-success-weak'),
      surfaceSuccessStrong: get('surface-success-strong'),
      surfaceWarningBase: get('surface-warning-base'),
      surfaceWarningWeak: get('surface-warning-weak'),
      surfaceWarningStrong: get('surface-warning-strong'),
      surfaceCriticalBase: get('surface-critical-base'),
      surfaceCriticalWeak: get('surface-critical-weak'),
      surfaceCriticalStrong: get('surface-critical-strong'),
      surfaceInfoBase: get('surface-info-base'),
      surfaceInfoWeak: get('surface-info-weak'),
      surfaceInfoStrong: get('surface-info-strong'),
      ...
      <header className="h-10 shrink-0 flex items-center justify-between px-3 gap-2 transition-colors relative" style={{ backgroundColor: theme.background, borderBottom: `1px solid ${theme.borderWeak}` }}>
        ...
        <div className="flex items-center gap-2">
          <div className="px-3 h-7 flex items-center justify-center rounded text-[12px] font-medium hover:opacity-90 active:opacity-100 cursor-pointer transition-colors shadow-sm"
            style={{ backgroundColor: theme.surfaceBrandBase, color: theme.textOnBrand }}>
            Share
          </div>
      ...
      >
              ...
              {sidebarItems.map((item, idx) => (
                <div
                  ...
                  style={{
                    ...
                    color: item.active ? theme.surfaceBrandBase : theme.iconWeak
                  }}
                >
                  ...
                  {item.active && (
                    <div className="absolute -left-[12px] w-1.5 h-6 rounded-r-full" style={{ backgroundColor: theme.surfaceBrandBase }} />
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden transition-colors" style={{ backgroundColor: theme.background }}>
          ...
          <div className="flex h-10 shrink-0 border-b items-center px-1 gap-0 transition-colors" style={{ backgroundColor: theme.background, borderColor: theme.borderWeak }}>
            <div className="flex items-center gap-2 px-4 h-full border-b-2 relative group cursor-pointer" style={{ borderBottomColor: theme.surfaceBrandBase, backgroundColor: theme.backgroundStrong }}>
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
                        ...
                        backgroundColor:
                          line.type === 'added' ? theme.diffAddBackground :
                            line.type === 'removed' ? theme.diffRemoveBackground :
                              line.type === 'hunk' ? theme.backgroundStrong : // Using backgroundStrong as a proxy for hunk background
                                'transparent',
                        color:
                          line.type === 'added' ? theme.diffAddForeground :
                            line.type === 'removed' ? theme.diffRemoveForeground :
                              line.type === 'header' || line.type === 'hunk' ? theme.foregroundWeak :
                                theme.foreground
                      }}
                    >
                      {/* Diff Line Prefix (+/-/@@) */}
                      ...
                      <span className="flex-1 whitespace-pre">
                        {line.type === 'header' || line.type === 'hunk' ? (
                          <span style={{ color: theme.foregroundWeak }}>{line.content}</span>
                        ) : (
                          highlightCode(line.content)
                        )}
                      </span>
                      {line.type === 'added' && (
                        <div className="absolute left-0 top-0 bottom-0 w-1" style={{ backgroundColor: theme.surfaceSuccessBase }} />
                      )}
                      {line.type === 'removed' && (
                        <div className="absolute left-0 top-0 bottom-0 w-1" style={{ backgroundColor: theme.surfaceCriticalBase }} />
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div className="h-48 border-t flex flex-col transition-colors shadow-[0_-4px_12px_rgba(0,0,0,0.1)]"
              ...
              <div className="h-9 shrink-0 flex items-center px-4 border-b transition-colors"
                style={{ borderColor: theme.borderWeak, backgroundColor: theme.backgroundStronger }}>
                <div className="flex items-center gap-2 border-b-2 h-full" style={{ borderBottomColor: theme.surfaceBrandBase }}>
